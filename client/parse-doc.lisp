;; Load/compile this file then run:
;; (swf-parse-doc::define-all-actions)
;; (swf-parse-doc::define-all-types)
...
(defpackage :swf-parse-doc
  (:use :common-lisp :html5-parser))

(in-package :swf-parse-doc)
             ...
             ((equal "dd" (node-name elt))
              (let* ((type-elt (find-node-if elt
                     ...
                     (type (parse-type-info (format nil "~A~A"
                                                     (node-value type-elt)
                                                     sub-type)))
                     (required (when required-elt (parse-required-info (node-value required-elt))))
...
(defun print-define-swf-type-form (form stream)
  (destructuring-bind (def name &rest defs)
      form
    (format stream "(~A ~S~{~%  ~S~})" def name defs)))

(defun print-define-swf-action-form (form stream)
  (destructuring-bind (def name request &rest response)
      form
    (format stream "(~A ~S~%    (~{~S~^~%     ~})~{~%  ~S~})" def name request response)))

(defun define-types (&rest types)
  (with-open-file (out (asdf:system-relative-pathname :cl-swf "client/types.lisp")
                       :direction :output
                       :if-exists :supersede)
    (write-line ";;; Do not edit. This file is generated by parse-doc.lisp." out)
    (prin1 '(in-package :swf) out)
    (terpri out)
    (dolist (type types)
      (format t "Type: ~A~%" type)
      (terpri out)
      (print-define-swf-type-form (parse-type type) out)
      (terpri out))))
...
(defun define-actions (&rest actions)
  (with-open-file (out (asdf:system-relative-pathname :cl-swf "client/actions.lisp")
                       :direction :output
                       :if-exists :supersede)
    (write-line ";;; Do not edit. This file is generated by parse-doc.lisp." out)
    (prin1 '(in-package :swf) out)
    ...
    (loop for (action emptyp) on actions by #'cddr do
          ...
          (terpri out)
          (print-define-swf-action-form (parse-action action emptyp) out)
          (terpri out))))
